...
def build():
    ...
    affiliates=load('integration/affiliate_networks_status.json'); masv2=load('integration/masoffer_v2_status.json'); ecotrack=load('integration/ecomobi_tracking_status.json')
    ...
    metrics={
      ...
      'at_offers':num(atcat.get('active_offer_count')),'at_tiktok':num(at.get('tiktok_product_count')),'masoffer_candidates':num(masv2.get('candidate_rows')),
      'ecomobi_candidates':num(eco.get('candidate_rows')),'ecomobi_recent_conversions':num(ecotrack.get('recent_conversion_rows'))
    }
    ...
    mas_state=masv2.get('state') or 'UNKNOWN'; eco_offer_state=eco.get('state') or 'UNKNOWN'; eco_track_state=ecotrack.get('state') or 'UNKNOWN'
    lines=[
      ...
      f"Ecomobi: tracking {eco_track_state} | offer {eco_offer_state} | candidates {metrics['ecomobi_candidates']}",
    ...
    if eco_track_state=='CREDENTIALS_INCOMPLETE': alerts.append('Ecomobi tracking: cần Token + Token Private')
    elif eco_track_state=='AUTH_NOT_RESOLVED': alerts.append('Ecomobi tracking: credential có nhưng auth variant chưa resolve')
    if eco_offer_state=='API_NOT_RESOLVED': alerts.append('Ecomobi offer feed: endpoint kho offer/product chưa resolve; tracking API không được tính là promo feed')
