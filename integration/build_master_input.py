...
from source_taxonomy import (
    consumer_category_for,
...
def main():
    ...
    def add(url=None, domain=None, name=None, origin=None, via=None, extra=None):
        ...
        if url:
            ...
            if a:
                g["access_observations"][a] += 1
        if name:
            g["names"].add(str(name).strip())
        if origin:
            g["origins"].add(origin)
        if via:
            g["discovery_methods"].add(via)
        extra = extra or {}
        fb, lb = extra.get("first_batch"), extra.get("last_batch")
        if isinstance(fb, int):
            g["legacy_first_batch"] = fb if g["legacy_first_batch"] is None else min(g["legacy_first_batch"], fb)
        if isinstance(lb, int):
            g["legacy_last_batch"] = lb if g["legacy_last_batch"] is None else max(g["legacy_last_batch"], lb)
    ...
    for path, origin in ((PROMO_EXPORT, "PROMO_CANONICAL_EXPORT"), (LEGACY_SEED, "PROMO_LEGACY_SEED")):
        ...
        for line in path.read_text(errors="replace").splitlines():
            ...
            add(
                url=url,
                domain=domain,
                name=r.get("source_brand") or r.get("brand") or r.get("name"),
                origin=origin,
                via=r.get("via") or r.get("source_type"),
                extra=r,
            )
    ...
    for rd, g in groups.items():
        ...
        category = consumer_category_for(rd, names, eps)
        vertical = vertical_for(rd, names, eps)
        market_score = market_relevance_score(
            rd, names, eps, g["origins"], g["discovery_methods"], catalog_domains
        )
        ...
        catalog_target = rd in catalog_domains
        priority = (
            45
            + min(25, promo_ep * 5)
            + (12 if "PROMO_CANONICAL_EXPORT" in g["origins"] else 0)
            + (15 if catalog_target else 0)
            + (market_score - 50) // 10
            - (20 if radar else 0)
        )
