...
create table if not exists public.promo_master_state (
  ...
  registered_sources integer not null,
  scanned_sources integer not null,
  actionable_offers integer not null,
  literal_codes integer not null,
...
create table if not exists public.promo_master_commits (
  ...
  registered_delta integer not null default 0,
  scanned_delta integer not null default 0,
  offers_delta integer not null default 0,
  codes_delta integer not null default 0,
  created_at timestamptz not null default now()
);
...
-- No public policies are intentionally created. Server-side service-role access only.
...
create or replace view public.promo_writer_health as
select
  s.batch_no,
  s.checkpoint,
