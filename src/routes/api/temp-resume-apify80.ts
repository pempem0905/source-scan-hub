...
const ONE_SHOT = "resume80-20260816";
...
export const Route=createFileRoute("/api/temp-resume-apify80")({server:{handlers:{POST:async({request})=>{
  try{
    ...
    const runInput={...oldInput,workerActorId:worker.id,dailyBudgetUsd:40,projectBudgetUsd:80,maxConcurrentJobs:Math.max(2,Number(limits?.limits?.maxConcurrentActorJobs??32)),displayBaseUrl:DISPLAY_BASE_URL,displayToken:secret("SOURCE_WORKER_TOKEN")};
    ...
    return Response.json({ok:true,dailyBudgetUsd:40,projectBudgetUsd:80,budget,schedule:{id:updated.id,enabled:updated.isEnabled,nextRunAt:updated.nextRunAt},limits:{max:limits?.limits?.maxConcurrentActorJobs,active:limits?.current?.activeActorJobCount},run:{id:run.id,status:run.status}});
